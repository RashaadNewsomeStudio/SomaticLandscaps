...
public class StartupPanelsFlow
{
    ...
    public async Task RunAsync(CancellationToken ct, System.Func<CancellationToken, Task> onComplete)
    {
        if (_ctrl.introPanel && _ctrl.introGroup)
        {
            ...
            await _ctrl.introGroup.FadeAlpha(0f, 1f, _ctrl.introFadeIn, ct);
            ...
            await _ctrl.introGroup.FadeAlpha(1f, 0f, _ctrl.introFadeOut, ct);
        ...
        if (_ctrl.adjustPanel && _ctrl.adjustGroup)
        {
            ...
            await _ctrl.adjustGroup.FadeAlpha(0f, 1f, _ctrl.adjustFadeIn, ct);
            ...
            await _ctrl.adjustGroup.FadeAlpha(1f, 0f, _ctrl.adjustFadeOut, ct);
            _ctrl.adjustPanel.SetActive(false);
        }

        if (onComplete != null) await onComplete(ct);
    }
